...
4. **Entailment (MUST).** A verifier that is **not** the writer fetches each cited passage and answers, factored (without the draft in context), "does this source — and only this — support this exact statement?" Fail any sentence its citations do not entail. Do not let the writer grade its own citations. A source that merely echoes the claim (SEO mirror, a page quoting the model's own output) is not support — the passage must be the source's own assertion. **When a second model family is available (e.g. Codex/GPT), run this check with it too and hard-fail on cross-family agreement** — a same-model verifier shares the writer's blind spots (the eval's #1 threat); method diversity is the fix.
