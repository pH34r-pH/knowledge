...
4. **NLI entailment per claim** [MUST]. A verifier subagent fetches each cited passage and answers the AIS test — "According to the source, does this passage (and only this) support this exact statement?" — computing citation recall (sentence fully entailed) and precision (each citation is actually needed) [3][5]. Even ChatGPT/GPT-4 leave ~50% of long-form answers not fully supported by their own citations [3], so this is not optional. **Anti-circular guard:** a source that merely restates the claim (an SEO mirror, a page quoting the model's own output, a citation-of-a-citation) is not support — the entailing passage must be the source's *own* assertion, not an echo of the claim being checked.
...
8. **The verifier is not the writer — ideally not the same model family** [MUST + SHOULD]. Run verification as a separate agent, answering its checks *factored* — without the original draft in context — so it cannot re-affirm the writer's own hallucination. Chain-of-Verification shows factored verification beats joint precisely because attending to the prior generation makes a model repeat its own errors; self-preference bias makes a model grading its own output systematically optimistic [7]. **A same-model verifier still shares the writer's systematic blind spots** (the flow's own eval named this its #1 threat). So when a second model family is available (e.g. Codex/GPT alongside Claude), run the entailment/contradiction check with it too and hard-fail on *cross-family agreement*, surfacing disagreements for a human — this is the method-diversity fix (different model/method/oracle, not same-model skepticism), not just a second instance of the same model.
...
11. **Tier sources by authority** [SHOULD]. Rank each source: peer-reviewed / primary > official standard or docs > reputable engineering writeup > vendor blog > forum post. This makes the "recurs across multiple independent *authoritative* sources" bar checkable rather than asserted, and flags a load-bearing claim that rests only on a low-tier source for extra scrutiny. Tier is a prior on trust, not a substitute for resolution + entailment.
...
- **The model layer shares blind spots with the generator.** AttrScore shows the topical-but-contradicting case is exactly where even a strong judge fails — a fraction of misgrounded citations will pass even a separate verifier [10]. The cross-model-family verifier (guard 8) is the mitigation; without a second family available, a same-model verifier only partially reduces this and the residual stands.
