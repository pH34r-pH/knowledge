...
**Benchmarks decay under their own success.** Once a public benchmark becomes prestigious it becomes a target, and Goodhart's law applies: contamination, task-specific tricks, and selective reporting inflate scores without improving capability [7]. Defenses are hidden/held-out test sets, data-provenance tracking, and dynamic or rotating benchmarks, so a score reflects generalization rather than memorization of a static bank [7][8].
