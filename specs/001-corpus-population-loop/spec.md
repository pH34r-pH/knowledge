...
- **FR-008**: System MUST commit only the files it changed, by explicit pathspec, and MUST push to the writable backup remote (`backup`) as the defined end of every run — this is a durable, extensible knowledge base, so a run is not complete until it is backed up to a remote. It MUST NOT attempt to push to a remote it lacks write access to (`origin`/the canonical repo) until access is granted, and MUST report a rejected push plainly rather than dropping it silently
