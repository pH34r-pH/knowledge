...
- **FR-014**: System MUST bias topic selection toward *recency / marginal value to a model with a training cutoff* — recent and fast-moving topics rank above saturated foundational ones, since the model already holds the foundational material (per [corpus/EVAL-corpus-leverage-2026-07-01.md](../../corpus/EVAL-corpus-leverage-2026-07-01.md), the marginal lift is largest where the model knows least). This is a steering bias, not a hard filter: a foundational topic still qualifies with a contested or freshly-synthesized angle. Recent topics MUST use a live-retrieval harness (`deep-research`/`storm`), never `vault-adapt`
